...
const Carousel = (props: CarouselProps) => {
  ...
  const arrowButtonClass = 'cursor-pointer mx-auto transition-all duration-200 hover:scale-110 hover:text-[var(--accent-hover)]';

  return (
    <div className="flex justify-between">
      <div className="hidden md:block my-auto w-16 h-16 rounded-full text-theme-accent">
        <ChevronLeftIcon
      ...
      <div className="md:w-4/5">
        ...
        <div className="flex justify-center space-x-2 mt-2">
          <div className="md:hidden my-auto w-8 h-8 rounded-full text-theme-accent">
            <ChevronLeftIcon
          ...
          {children.map((_, i) => {
            const baseCircle = 'w-4 h-4 mt-3 border-2 border-theme-accent rounded-full cursor-pointer self-center transition-all duration-200 hover:border-[var(--accent-hover)] hover:scale-110';
            const circle = i === slideIndex ? `${baseCircle} bg-theme-accent` : baseCircle;
            return (
              <div
